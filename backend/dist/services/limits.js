...
export async function getUserLimits(userId, kycStatus) {
    ...
    if (kycStatus === 'approved')
        return { daily_limit_usd: 2000, monthly_limit_usd: 10000 };
    return { daily_limit_usd: 500, monthly_limit_usd: 1000 };
}
export async function getUserSpent(userId) {
...
export async function enforceLimits(userId, kycStatus, amount) {
    ...
    if (spent.monthly + amount > limits.monthly_limit_usd) {
        const err = new Error('Monthly limit exceeded');
        err.code = 'LIMIT_EXCEEDED';
        err.status = 400;
        throw err;
    }
